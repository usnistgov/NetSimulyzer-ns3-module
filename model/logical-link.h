...
/**
 * Representation of some abstract 'link'
 * or logical connection between two
 * configured Nodes
 */
class LogicalLink : public Object
{
  public:
    ...
    /**
     * Creates an unmanaged LogicalLink.
     *
     * This constructor is used by the helper,
     * `m_ignoreSets` must be set to `true` and
     * `m_orchestrator->CreateLink(*this)` must be called by hand
     ...
     * removes it from view in the application
     *
     * Does nothing if it's already inactive
     */
    void Deactivate();

    /**
     * Toggles the link from Active to Inactive
     * and vice versa.
     */
    void Toggle();
