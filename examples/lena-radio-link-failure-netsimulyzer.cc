...
uint32_t oldByteCounter = 0;
  ...
  void NotifyReportUeMeasurements (std::string path, uint16_t rnti, uint16_t cellId, double rsrp, double sinr, bool serving, uint8_t ccId)
  {
                               ...
                               uint16_t rnti)
{
  ...
  {
    *ueLog << Simulator::Now ().GetSeconds () << " " << context
            << " UE IMSI " << imsi
            << ": connected to cell id " << cellid
            << " with RNTI " << rnti;
  }
                                ...
                                uint16_t rnti)
{
  ...
  if (enableVisualization)
  {
    *enbLog << Simulator::Now ().GetSeconds () << " " << context
            << " eNB cell id " << cellId
            << ": successful connection of UE with IMSI " << imsi
            << " RNTI " << rnti;
  }
...
EnbRrcTimeout (uint64_t imsi, uint16_t rnti, uint16_t cellId, std::string cause)
{
  ...
  if (enableVisualization)
  {
    *enbLog << Simulator::Now ().GetSeconds ()
            << " IMSI " << imsi << ", RNTI " << rnti << ", Cell id " << cellId
            << ", ENB RRC " << cause;
  }
#endif
...
NotifyConnectionReleaseAtEnodeB (uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
  ...
  if (enableVisualization)
  {
    *enbLog << Simulator::Now ().GetSeconds ()
            << " IMSI " << imsi << ", RNTI " << rnti << ", Cell id " << cellId
            << ", UE context destroyed at eNodeB";
  }
#endif
}
...
void RadioLinkFailure (Time t310, uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
  ...
  if (enableVisualization)
  {
    *ueLog << Simulator::Now ().GetSeconds ()
            << " IMSI " << imsi << ", RNTI " << rnti
            << ", Cell id " << cellId << ", radio link failure detected";
  }
#endif
